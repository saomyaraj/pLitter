...
def draw_boxes_and_count_on_image(image, boxes, classes, class_ids, scores, use_normalized_coordinates=False, min_score_thresh=0.3):
    """
    Draw bounding boxes on the image and overlay a count of detected objects.
    Ignores objects of the class 'Trash bin'.
    """
    count = 0
    for i in range(len(boxes)):
        box = boxes[i]
        ...
        if score >= min_score_thresh:
            ...
            image = cv2.rectangle(image, (x1, y1), (x2, y2), colors[class_id], 2)
            cv2.putText(image, f"{category}:{round(score,2)}", (x1, y1 - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors[class_id], 1)
            # Count this detection if it's not a 'Trash bin'
            if category.lower() != 'trash bin':
                count += 1
    # Overlay the count on the top-left corner of the image
